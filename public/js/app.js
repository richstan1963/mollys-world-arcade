...
window.arcade = {
    ...
    async playGame(romId) {
        ...
        try {
            ...
            // ─── Desktop Mode: native emulator launch ──────────────────────────
            if (config.isNative) {
                overlay.style.display = 'flex';
                container.innerHTML = `<div class="desktop-mode-loading">
                    <div class="dm-spinner">⚡</div>
                    <div class="dm-loading-text">Preparing Desktop Mode…</div>
                </div>`;

                try {
                    const dInfo = await fetch(`/api/desktop/launch-url/${romId}`).then(r => r.json());
                    const accentColor = dInfo.color || '#00D4FF';

                    container.innerHTML = `
                    <div class="desktop-mode-panel">
                        <div class="dm-header">
                            <div class="dm-icon">🖥️</div>
                            <div class="dm-header-text">
                                <div class="dm-mode-label">DESKTOP MODE</div>
                                <div class="dm-title">${H.escHtml(dInfo.title)}</div>
                            </div>
                        </div>

                        <div class="dm-emulator-card" style="--dm-accent:${accentColor}">
                            <div class="dm-emulator-name">${dInfo.emulator}</div>
                            <div class="dm-quality-badge">${dInfo.quality}</div>
                            <div class="dm-system-tag">${H.escHtml(dInfo.systemName || dInfo.system)}</div>
                        </div>

                        <div class="dm-info">
                            <p>Your arcade launches <strong>${dInfo.emulator}</strong> natively — RTX 3080 running at full power.</p>
                            <p class="dm-stream-note">💾 ROM streams automatically to your local cache on first launch.</p>
                        </div>

                        <div class="dm-actions">
                            <button class="dm-launch-btn" id="dm-launch-btn">
                                <span class="dm-launch-icon">🚀</span>
                                Launch in ${H.escHtml(dInfo.emulator)}
                            </button>
                            <button class="dm-copy-btn" id="dm-copy-btn">
                                📋 Copy URL
                            </button>
                        </div>

                        <div class="dm-footer">
                            <span>Requires <strong>Arcade Launcher</strong> installed on this machine</span>
                            <a href="#" onclick="window.arcade.closePlayer();return false;" class="dm-close-link">✕ Close</a>
                        </div>
                    </div>`;
                // Bind launch/copy buttons safely (no inline onclick with unsanitized URLs)
                const launchBtn = container.querySelector('#dm-launch-btn');
                const copyBtn = container.querySelector('#dm-copy-btn');
                if (launchBtn) launchBtn.addEventListener('click', () => { window.location.href = dInfo.launchUrl; });
                if (copyBtn) copyBtn.addEventListener('click', () => { navigator.clipboard.writeText(dInfo.launchUrl).then(() => H.toast('Launch URL copied', 'success')); });
                } catch (err) {
                    container.innerHTML = `<div class="desktop-mode-panel">
                        <div class="dm-header"><div class="dm-icon">🖥️</div>
                            <div class="dm-header-text">
                                <div class="dm-mode-label">DESKTOP MODE</div>
                                <div class="dm-title">${H.escHtml(config.gameName)}</div>
                            </div>
                        </div>
                        <div class="dm-info"><p style="color:#f87171">Could not fetch launch URL. Make sure the server is running.</p></div>
                        <div class="dm-footer"><a href="#" onclick="window.arcade.closePlayer();return false;" class="dm-close-link">✕ Close</a></div>
                    </div>`;
                }
                return;
            }
